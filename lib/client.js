...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  xml = "<soap:Envelope " +
    ...
    ((self.soapHeaders || (self.security && self.security.toXML())) ?
      (
        "<soap:Header>" +
  ...
  var reqCtx = {};
  reqCtx.xml = xml;
  self.emit('beforeRequest', reqCtx);
  self.emit('request', reqCtx.xml);

  req = self.httpClient.request(location, reqCtx.xml, function(err, response, body) {
    var result;
    var obj;
    ...
    var resCtx = {};
    resCtx.body = body;
    self.emit('afterResponse', resCtx);
    self.emit('response', resCtx.body);

    if (err) {
      callback(err);
    } else {
      try {
        obj = self.wsdl.xmlToObject(resCtx.body);
      } catch (error) {
        ...
        if(output && !output.$lookupTypes) {
          ...
          return callback(null,response,resCtx.body,null);
        }
        error.response = response;
        error.body = body;
        self.emit('soapError', error);
        return callback(error, response, resCtx.body, null);
      }

      if (!output){
        // one-way, no output expected
        return callback(null, null, resCtx.body, obj.Header);
      }
      ...
      if( typeof obj.Body !== 'object' ) {
        ...
        error.response = response;
        error.body = body;
        return callback(error, obj, resCtx.body, null);
      }
      ...
      callback(null, result, resCtx.body, obj.Header);
    }
